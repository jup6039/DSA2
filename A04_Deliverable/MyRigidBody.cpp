...
{
	// Variables
	matrix4 modelMatrixA = GetModelMatrix();
	matrix4 modelMatrixB = a_pOther->GetModelMatrix();
	vector3 extentsA = m_v3HalfWidth;
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			R[i][j] = glm::dot(modelMatrixA[i], modelMatrixB[j]);
		}
	}

	// Translation Vector
	vector3 t = centerB - centerA;
	t = vector3(glm::dot(t, vector3(modelMatrixA[0])), glm::dot(t, vector3(modelMatrixA[1])), glm::dot(t, vector3(modelMatrixA[2])));
