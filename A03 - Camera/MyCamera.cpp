...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	vector3 m_v3Forward = glm::normalize(m_v3Target - m_v3Position);	// Calculate forward vector
...
{
	vector3 m_v3Up = glm::normalize(m_v3Above - m_v3Position);		// Calculate up vector
...
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	vector3 m_v3Up = glm::normalize(m_v3Above - m_v3Position);
	vector3 m_v3Right = glm::cross(m_v3Up, m_v3Forward);		// Calculate right vector
...
void MyCamera::RotateVertical(float a_fAngle)		// Yaw
{
	vector3 m_v3Forward = glm::normalize(m_v3Target - m_v3Position);	// Forward vector
	quaternion m_qRotation = glm::angleAxis(a_fAngle, AXIS_Y);			// Quaternion

	m_v3Forward = m_qRotation * m_v3Forward * glm::conjugate(m_qRotation);
	m_v3Target = m_v3Position + m_v3Forward;
}

void MyCamera::RotateHorizontal(float a_fAngle)		// Pitch
{
	vector3 m_v3Forward = glm::normalize(m_v3Target - m_v3Position);	// Forward vector
	vector3 m_v3Up = glm::normalize(m_v3Above - m_v3Position);			// Up vector
	vector3 m_v3Right = glm::cross(m_v3Up, m_v3Forward);				// Right vector
	quaternion m_qRotation = glm::angleAxis(a_fAngle, m_v3Right);		// Quaternion
